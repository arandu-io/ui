...
22 files. 13 views, and 9 plain Go: four controllers in
`app/Http/Controllers/Auth/` (`LoginController.go`,
`LoginController_handlers.go`, `RegisterController.go`,
`PasswordController.go`), `render.go` and `page.go` beside them, the two
mailables in `app/Mail/`, and `app/Http/Controllers/HomeController.go`.

`GenerateAuth` in `views_controllers.go:18` is the list of the nine Go files;
`AuthViews` in `views.go:55` is the list of the thirteen views plus `page.go`.
There are exactly 22 template constants, one per published file
(`grep -ho 'const [a-zA-Z]*Template' views*.go | wc -l`), and 22 golden files
under `testdata/auth/`.
...
`write` in `publish.go:230` reads the file on disk — it does not stat it — and
...
Publishing twice into the same project, with no flag, writes 5 and keeps 17.
...
render. `replaced` in `publish.go:139` spells the five out rather than inferring
...
16 of the 22. The 13 views, plus the three Go files the layout unit does not
compile without: `page.go`, `render.go` and `HomeController.go`. What it leaves
alone is the flow — the four controllers and the two mailables, which are the
files somebody edits to decide their own rules.
...
`screensOnly` in `views.go:872` is the filter, and
...
markers, and a republish carries it forward. 9 of the 22 published files have
...
printed to the reader of an e-mail. `markerFor` at `publish.go:175` picks by
...
`merge` at `publish.go:188` matches blocks **by position, not by name**. That is
...
4. If it mounts a route, the route and its name go in the table at
   `flow_internal_test.go:940`, which is exact and ordered on purpose.
5. Run the gates.
...
`require` here is something they download to publish 22 files. A CI step fails
