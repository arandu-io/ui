...
func SignedInName(ctx context.Context, people *auth.Service, tenant, id string) string {
	...
	// string, so the policy sees who is asking. The reader here is the person
	// being greeted, scoped to the tenant the caller passed.
	names, err := people.PublicNames(ctx, security.Subject{Tenant: tenant}, []string{id})
	if err != nil || names[id] == "" {
		return id
	}
	return names[id]
...
// fragment answers one part of a screen when htmx asked for one, and the whole
// screen otherwise.
//
// A control that carries hx-target is asking for its own markup back, not a
// document. Answering a page into that hole puts the header, the navigation and
// a second toaster inside it -- the status is right, the build is green, and the
// screen looks like it rendered twice. So the two names are given together here:
// what the address draws on its own, and what one control of it draws.
//
// The branch is content negotiation and not a second way to answer. It is the
// same question fhttp.Redirect already asks for itself, and it is asked once,
// here, so a handler never has to.
//
// Boosted requests take the page. hx-boost swaps the whole body, so a fragment
// answered to one leaves the person on a document that is a bare form -- no
// header, no navigation, no way out. It cannot happen while the form carries an
// explicit hx-post, and it is one line to keep it from happening to whoever
// deletes that attribute.
//
// # What a caller may fill
//
// Only what the part draws. The two names are one view and a piece of it, and
// on the branch that answers the piece the rest of the screen is not rendered
// at all -- so a field the screen draws and the part does not is a value this
// call sends and nobody ever sees. Nothing reports it: the status is right, the
// markup in the hole is right, and the sentence is gone.
//
// The chrome is the same story one level out. data.Page is filled on every
// path because the screen needs it on the other branch, and on this one the
// layout is not re-rendered either, so what the header shows is what it showed
